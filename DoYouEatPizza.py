...
self.df2 = pd.DataFrame.from_records([{'메뉴': '피자', '개수': 1, '가격': 3500}])
pizza_tem = pd.read_excel("pizzaInfo.xlsx", sheet_name="Sheet2")


def addResultData(date, name, amount, price):

        new_row = {'날짜': date, '메뉴': name, '개수': amount, '가격': price}
        self.df1 = self.df2.append(new_row, ignore_index=True)

        self.df1.to_excel("pizzaInfo.xlsx", sheet_name='Sheet1')
        print(self.df1)
...
def addTemData(name,amount, price):
        ...
        self.df2.to_excel("pizzaInfo.xlsx", sheet_name='Sheet2')
...
class MainWindow(QMainWindow):
    ...
    def clickPre(self):
        ...
        pre_widget.pz1Btn.clicked.connect(lambda img_name="starChefSignature", name="스타 셰프 시그니처": self.click_menu(img_name, name))
        pre_widget.pz2Btn.clicked.connect(self.clickPz1)
        pre_widget.pz3Btn.clicked.connect(self.clickPz1)
        pre_widget.pz4Btn.clicked.connect(self.clickPz1)
        pre_widget.pz5Btn.clicked.connect(self.clickPz1)
        pre_widget.pz6Btn.clicked.connect(self.clickPz1)

        self.central_widget.addWidget(pre_widget)
        self.central_widget.setCurrentWidget(pre_widget)

    def click_menu(self, img_name, name):
        about_widget = AboutWidget(self)
        about_widget.setting(img_name, name)
    ...
    def clickSub_side(self):
        ...
        rec_widget.rec1Btn.clicked.connect(
            lambda img_name="tomato_pasta", name="셰프’s 토마토 파스타", price=9800: self.click_menu(img_name, name, price))
        rec_widget.rec2Btn.clicked.connect(
            lambda img_name="basil_pasta", name="셰프’s 트러플 바질 파스타", price=9800: self.click_menu(img_name, name, price))
        rec_widget.rec3Btn.clicked.connect(
            lambda img_name="rose_pasta", name="스파이시 씨푸드 로제 파스타", price=8800: self.click_menu(img_name, name, price))
        rec_widget.rec4Btn.clicked.connect(
            lambda img_name="crispy_chicken", name="크리스피 핫 순살 치킨(8조각)", price=4800: self.click_menu(img_name, name,
                                                                                                    price))
        rec_widget.rec5Btn.clicked.connect(
            lambda img_name="salad", name="샐러드 가든", price=6800: self.click_menu(img_name, name, price))
        rec_widget.rec6Btn.clicked.connect(
            lambda img_name="penne_pasta", name="펜네 파스타", price=8800: self.click_menu(img_name, name, price))
...
class OrderWidget(QWidget):
    ...
    #def showDB(self):
     #   print(str(pizza_tem[['메뉴', '개수', '가격']]))
      #  self.cart.setText(str(pizza_tem[['메뉴','개수','가격']]))
...
class AboutWidget(QWidget):
    ...
    def setting(self, img_name, name):
        ...
        self.pzPic.setPixmap(QPixmap("images/"+img_name+".png"))
    ...
    def clickCart(self):
        ...
        if option == QMessageBox.Yes:  # 버튼의 이름을 넣으면 됩니다.
            ...
            addTemData(self.pzName.text(), self.pizzaAmount, self.resultPrice)
    ...
    def getResultPrice(self):
        print(str(self.resultPrice))
...
class PreWidget(QWidget):
    def __init__(self, parent=None):
        ...
        self.pzName2 = QLabel("스타 시그니쳐", self)
        ...
        self.pzName3 = QLabel("스타 쉐프", self)
