...
class MainWindow(QMainWindow):
    ...
    def clickSub_side_next(self):
        ...
        rec_widget.rec1Btn.clicked.connect(
            lambda img_name="grain_chicken", name="슈퍼곡물 치킨(10조각)", price=7800: self.click_menu(img_name, name, price))
        rec_widget.rec2Btn.clicked.connect(
            lambda img_name="bolognese_spaghetti", name="NEW 치즈 볼로네즈 스파게티", price=8800: self.click_menu(img_name, name,
                                                                                                        price))
        rec_widget.rec3Btn.clicked.connect(
            lambda img_name="truffle_risotto", name="트러플 리조또", price=8800: self.click_menu(img_name, name, price))
        rec_widget.rec4Btn.clicked.connect(
            lambda img_name="half_spaghetti", name="하프&하프 스파게티 (NEW 치즈 & 화이트 크림)", price=9800: self.click_menu(img_name,
                                                                                                               name, price))
        rec_widget.rec5Btn.clicked.connect(
            lambda img_name="cream_spaghetti", name="화이트 크림 스파게티", price=8800: self.click_menu(img_name, name, price))
        rec_widget.rec6Btn.clicked.connect(
            lambda img_name="wings", name="갈릭&허브윙스(8조각)", price=8800: self.click_menu(img_name, name, price))
    ...
    def click_menu(self, img_name, name, price):
